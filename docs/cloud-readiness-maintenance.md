...
A further native failure-path check used the actual release and installed helper on a private display, without fixture data. Clicking cloud Connect created the editable pending panel in 104 ms. Native Enter cleared the editor and retained the exact prompt in the queue. The real AWS expired-session error appeared inline, `AWS access` was marked failed, all later steps remained waiting, and the timer stopped at one second. The same panel stayed open with no automatic local or remote fallback. Logs recorded one failed `wake-ready`, no SSH/proxy/model-sync actions, and no provider request. Native panel cleanup left zero test-owned processes. Evidence: `target/warm-transport-20260919/auth-fail-2108/result.json` and `auth-failure.png`. An earlier harness attempt stopped before Connect because its status-settling assertion incorrectly rejected a terminal authentication error; that assertion was corrected before this passing run. This verifies error handling and draft preservation, not successful warm-session latency.
