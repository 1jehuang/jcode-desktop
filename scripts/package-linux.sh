...
for command in cargo tar dpkg-deb install; do command -v "$command" >/dev/null || { echo "error: missing $command" >&2; exit 1; }; done
...
if [[ "${SKIP_BUILD:-0}" != 1 ]]; then
  ...
  cargo build --manifest-path "$JCODE_REPO/Cargo.toml" --release --bin jcode
  cargo build --manifest-path "$JCODE_REPO/Cargo.toml" --release --package jcode-harness-api-server --bin jcode-harness-api-bridge
fi

rm -rf "$STAGE" "$DEBROOT"; mkdir -p "$STAGE"
install -m755 "$ROOT/target/release/jcode-desktop" "$STAGE/"
install -m755 "$JCODE_REPO/target/release/jcode" "$STAGE/"
install -m755 "$JCODE_REPO/target/release/jcode-harness-api-bridge" "$STAGE/"
...
Depends: libxkbcommon0, libfontconfig1
