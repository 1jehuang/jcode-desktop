//! Headless checks of the tab-bar host close action.
use super::*;
...
#[gpui::test]
fn tab_close_window_action_retains_draft_and_refuses_failed_snapshot(cx: &mut TestAppContext) {
    let (manager, current) = setup(cx);
    cx.update(|cx| host::window_controls::install(cx, manager.clone(), current.clone()));
    let original = current.borrow().unwrap();
    cx.update(|cx| {
        cx.global_mut::<Failures>().snapshot = true;
        cx.dispatch_action(&host::window_controls::CloseWindow);
    });
    cx.run_until_parked();
    assert_eq!(current.borrow().unwrap().window_id(), original.window_id());
    assert_eq!(cx.windows().len(), 1);

    cx.update(|cx| cx.global_mut::<Failures>().snapshot = false);
    for _ in 0..3 {
        cx.update(|cx| {
            // Resolve by name exactly as a freshly loaded UI generation does.
            let action = cx
                .build_action("jcode_desktop_host::CloseWindow", None)
                .unwrap();
